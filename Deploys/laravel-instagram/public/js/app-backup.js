		...
		this.likeInteractive.forEach((e, i) => {
			e.addEventListener('click', () => {
				if(e.classList.contains('text-danger')){
					...
					fetch('/aomine/instagram/dislike/' + e.dataset.id, {
						method: 'get'
                	...
                	this.socialLikeCount[i].innerHTML = parseInt(this.socialLikeCount[i].innerHTML) + 1;
                	fetch('/aomine/instagram/like/' + e.dataset.id, {
                		method: 'get'
                	}).then(function(response){
                        // console.log(response.json());
                    }).catch(function(err){
...
class Comments {
    ...
    restSave(){
    	this.url = '/aomine/instagram/comment/save';
    	this.data = {
    ...
    restDelete(e){
    	fetch( '/aomine/instagram/comment/delete/' + e.dataset.id, {
    		method: 'get'
    	}).then(response => {
            // console.log(response.json());
        }).catch(err => {
